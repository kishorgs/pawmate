...
export class VeterinariansRepository {
  ...
  async createVeterinarian(input: CreateVeterinarianDto): Promise<VeterinarianDocument> {
    ...
    return { id: ref.id, ...(snapshot.data() as Omit<VeterinarianDocument, 'id'>) };
  }

  async findVeterinarianById(id: string): Promise<VeterinarianDocument | null> {
    const snapshot = await this.collection().doc(id).get();
    if (!snapshot.exists) return null;
    return { id: snapshot.id, ...(snapshot.data() as Omit<VeterinarianDocument, 'id'>) };
  }
